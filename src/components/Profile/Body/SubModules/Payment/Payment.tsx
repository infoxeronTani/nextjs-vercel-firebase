...
export default function PaymentSection() {
  ...
  const getContactInfo = async () => {
    try {
      ...
      if (response.ok && json.data.length > 0) {
        const formData: any = _.omit(json.data[json.data.length - 1], ['uid', 'metadata', 'createdAt']);
      ...
      if (_.isEmpty(error)) {
        const formData = {
          ...
          payload: {
            ...formFields,
            uid: user?.uid,
            fp: !_.isEmpty(existedContact) ? existedContact.fp : `contact_info_${Date.now()}`,
            metadata: {
  ...
  const makePayment = async () => {
    ...
    const data = await fetch('/api/merchant/razorpay', { method: 'POST' }).then((t) => t.json());
    var options = {
      ...
      handler: function (response: any) {
        ...
        const renewDays = moment(user?.subscriptionEndDate).diff(moment(), 'days')
        const body: ExpectedUpdaeUserPayload = {
          ...
          updatedUserData: {
            ...
            subscriptionEndDate: moment().add({ months: 4, days: renewDays > 0 ? renewDays : 0}).toISOString(),
          },
        };
  ...
  const formChangeHandler = (value: string | number | string[], key: string) => {
  ...
  if (
    ...
    const renewcondition = (moment(user?.subscriptionEndDate).diff(moment(), 'days') <= 5)
    probutton = (
  ...
  if (
    ...
    total = (
      <Typography
        ...
        component='h1'
      >
        <CurrencyRupeeIcon sx={{ mr: 1, fontSize: '110%', fontWeight: 'bold' }} /> 0
      </Typography>
    );
  } else if (
    user.plan &&
    ...
    total = (
      <Typography
        ...
        component='h1'
      >
        <CurrencyRupeeIcon sx={{ mr: 1, fontSize: '110%', fontWeight: 'bold' }} /> 1,770
      </Typography>
    );
  }
    ...
    <>
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={6}>
          <Box sx={{ minWidth: 275 }}>
          <Card variant='outlined' sx={{ py: 2,background: '#F8F8F8', boxShadow: '0px 10px 20px rgba(124, 124, 124, 0.2)', border:'0' }}>
          <Box sx={{ display: 'flex', px: 2 }}>
                {/* <CheckCircleIcon sx={{ color: '#4DAA0F', fontSize: '160%' }} /> */}
                <Box sx={{ width: '100%' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', pb: 2 }}>
                    <Box sx={{ ml: 1 }}>
                      <Typography sx={{ mb: 0 }} variant='subtitle1' component='h1'>
                        <b style={{fontStyle: 'normal',fontWeight: 700,fontSize: '21px',lineHeight: '29px'}}>Your order</b>
                      </Typography>  
                    </Box>
                  </Box>
                  <Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', py: 2, width:"100%"  }}>
                      <Box sx={{ ml: 1, width:'100%', display: 'flex', }}>
                        <Box sx={{ width:'50%' }}>
                          <Typography sx={{ mb: 0 }} variant='subtitle1' component='h1'>
                            <b style={{textTransform:'capitalize'}}>Break Free {plan}</b>
                          </Typography>
                          <Typography sx={{ mb: 0 }} variant='body2' gutterBottom component='h1'>
                         <b> Collaborate, Earn, Grow and be limitless with Break Free Pro.</b>
                          </Typography>
                          <Typography sx={{ fontSize: '12px'}}>
                          Plan Subscription :  <CurrencyRupeeIcon sx={{ fontSize: 11, fontWeight: 'medium' }} /> {plan === 'basic'? '0 for month':'1500 for 3 months'}
                          </Typography>
                       
                        </Box>
                        <Box  sx={{ width:'50%', textAlign: 'end', mt:5}}>
                            <h1 style={{ margin: 0 }}> <CurrencyRupeeIcon />&nbsp;{plan==='basic'? '0': '1500'}</h1>
                          </Box>
                      </Box>
                    </Box>
                  </Box>
            ...
            <Card variant='outlined' sx={{ py: 2, mt: 5,background: '#F8F8F8', boxShadow: '0px 10px 20px rgba(124, 124, 124, 0.2)', border:"0" }}>
              <Box sx={{ display: 'flex', px: 2 }}>
                ...
                <Box sx={{ width: '100%' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', pt: 2, pb: 2 }}>
                    <Box sx={{ ml: 1 }}>
                      <Typography sx={{ mb: 0 }} variant='body2' gutterBottom component='h1'>
                        <b className='cont_info' style={{fontWeight: 700,fontSize: '21px', lineHeight: '29px'}}>Contact Information</b>
                      </Typography>
                    </Box>
                  ...
                  <Box sx={{ ml: 1, py: 1 }}>
                    {isEditContact ? (
                      <>
                        ...
                        <div className={classes.editContactInfo}>
                          <LocationOnIcon />
                          <TextField disabled label='State' size='small' value={formFields.state} />
                        </div>
                      </>
                    ) : (
                      <>
                        <Typography sx={{ mb: 0 }} variant='body2' gutterBottom component='h1'>
                          {existedContact?.city ||
                            (user.city !== undefined && user.city[0].toUpperCase() + user.city.slice(1))}
                        </Typography>
                        <Typography sx={{ mb: 0 }} variant='body2' gutterBottom component='h1'>
            ...
            <Card variant='outlined' sx={{ p: 4, mt: 5,background: '#F8F8F8', boxShadow: '0px 10px 20px rgba(124, 124, 124, 0.2)', border:"0" }}>
            <Typography variant='subtitle1' gutterBottom component='div' sx={{ fontWeight: 700,fontSize: '21px',lineHeight: '29px',mb:3 }}>
              ...
              {includesWithSubscription.map(({ _id, title }) => (
                <Box key={_id} sx={{ display: 'flex' }}>
                 ...
                 <img src="/checkedimg.svg" height={20} width={20}/>
                  <Typography sx={{ml:2}} variant='subtitle1' gutterBottom component='div'>
                    {title}
                  </Typography>
                </Box>
              ))}
        ...
        <Grid item xs={12} md={6}>
          ...
          <Box sx={{ minWidth: 275, p:2 }}>
            <Card variant='outlined' sx={{ backgroundColor: '#FFFEE7',my:4, boxShadow: '0px 10px 20px rgba(124, 124, 124, 0.2)', px:3, border:"0" }}>
              ...
              >
                ...
                <Box sx={{ ml: 1 }}>
                  <Link
                    className={classes.buttonLink}
                    onClick={() => router.push(`/merchants/${user.slug}/merchant-nda/${plan}`)}
                    variant='body2'
                    gutterBottom
                    sx={{textDecoration: 'none'}}
                  >
              ...
              <Box sx={{ px: 2, py: 3 }}>
                <FormControlLabel
                  control={<Checkbox checked={ndaTrue} onChange={(e) => setNdaTrue(e.target.checked)} />}
                  label='I agree to the terms and condition.'
                  style={{ pointerEvents: `${isMerchants === true ? 'auto' : 'none'}` }}
                />
              </Box>
              {/* <Divider /> */}
              {/* <Box
              ...
              <Box sx={{ px: 4, py: 2, mb:5 }} style={{ pointerEvents: `${ndaTrue === true ? 'auto' : 'none'}` }}>
                {probutton}
              </Box>
            </Card>
          </Box>
